...
func collectEcNodesForDC(commandEnv *CommandEnv, selectedDataCenter string, toDiskType types.DiskType) (ecNodes []*EcNode, totalFreeEcSlots int, err error) {
	...
	ecNodes, totalFreeEcSlots = collectEcVolumeServersByDc(topologyInfo, selectedDataCenter, toDiskType)
...
func collectEcNodes(commandEnv *CommandEnv, toDiskType types.DiskType) (ecNodes []*EcNode, totalFreeEcSlots int, err error) {
	return collectEcNodesForDC(commandEnv, "", toDiskType)
}
...
func collectEcVolumeServersByDc(topo *master_pb.TopologyInfo, selectedDataCenter string, toDiskType types.DiskType) (ecNodes []*EcNode, totalFreeEcSlots int) {
	eachDataNode(topo, func(dc DataCenterId, rack RackId, dn *master_pb.DataNodeInfo) {
		...
		freeEcSlots := countFreeShardSlots(dn, toDiskType)

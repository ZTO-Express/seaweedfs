...
func GetAuthenticated(url, jwt, authHeader string) ([]byte, bool, error) {
	request, err := http.NewRequest(http.MethodGet, url, nil)
	if err != nil {
		return nil, true, err
	}
	maybeAddAuth(request, jwt, authHeader)
...
func DeleteProxied(url string, jwt, authHeader string) (body []byte, httpStatus int, err error) {
	req, err := http.NewRequest(http.MethodDelete, url, nil)
	maybeAddAuth(req, jwt, authHeader)
	if err != nil {
		return
...
func DownloadFile(fileUrl string, jwt, authHeader string) (filename string, header http.Header, resp *http.Response, e error) {
...
func ReadUrlAsStream(ctx context.Context, fileUrl, jwt, authHeader string, cipherKey []byte, isContentGzipped bool, isFullChunk bool, offset int64, size int, fn func(data []byte)) (retryable bool, err error) {
	if cipherKey != nil {
		return readEncryptedUrl(ctx, fileUrl, jwt, authHeader, cipherKey, isContentGzipped, isFullChunk, offset, size, fn)
	}
...
func readEncryptedUrl(ctx context.Context, fileUrl, jwt, authHeader string, cipherKey []byte, isContentCompressed bool, isFullChunk bool, offset int64, size int, fn func(data []byte)) (bool, error) {
	encryptedData, retryable, err := GetAuthenticated(fileUrl, jwt, authHeader)
	if err != nil {
